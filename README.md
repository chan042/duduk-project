...
* **자연어 입력**: "강남 스타벅스 아메리카노 4500 원"과 같이 일상 언어로 입력하면, AI가 자동으로 카테고리, 금액 등을 파싱 후 저장하는 기능.

* **영수증 OCR 스캔**: 종이 영수증을 촬영하면 광학 문자 인식(OCR) 기술을 통해 상호명, 일시, 품목별 금액 합계를 자동으로 디지털화하는 기능.

* **이미지-상품 매칭**: 사용자가 음식점 사진을 업로드하면 AI가 이미지를 분석 후 해당 가게의 메뉴 가격 정보를 검색하고 사용자가 소비한 메뉴와 매칭하는 기능.
